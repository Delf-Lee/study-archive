...
## 느낀점
어번 미션에 대해 전체 피드백이 나왔다. 지난 주와 같이 신경쓰이는 점에 대해서 정리하겠다.
### 기능 목록 구현을 재검토한다.
확실히 지저분한 한것 같다. 애초에 이렇게 목록을 README에 작성해 나가는 것에 익숙하지 않아서 그런 것 같다. 클래스와 메서드명을 포함한 고유명사의 사용을 자제하고 조금 더 명확하게 간결히 작성해보도록 노력해야겠다.

### 배열 대신 java collection을 사용하라
우연히도 이번에는 배열 대신 `List`로 구현한 부분이 꽤 있다. 평소에는 내가 충분히 이해하고 메모리를 컨트롤 할 수 있는 상황이라 판단하고 배열을 쓰는 것을 선호한다. 성능상 효율이 좋기 때문이다. 물론 내가 짜고 있는 레벨에서는 그런 최적화는 필요없는 수준이라는걸 알고 있지만... 웬지 모르게 닭잡는데 '소 잡는 칼로 닭 잡는' 기분을 떨칠수가 없다. 하지만 현재 공부하고 있는 [이펙트브 자바]에서도 그렇고 지금도 그렇고 여러 곳에서 컬렉션을 쓰는 것을 권장한다. 아마 안정성과 범용성, 확장성면에서 너 나은 선택이기 떄문이겠지...(제네릭이라던가) 

앞으로는 조금 더 생각해보고 사용해야겠다.

### 함수(메서드) 라인에 대한 기준
피드백에서는 말한다.
> "공백, 주석도 한 라인이다. 주석도 되도록 코드 우측에 달아라."

나름 신선한 충격이었다. 이런 컨벤션에 대해서는 딱히 나만의 무언가를 가지고 있지는 않았지만, 조금 의외인 감이 있었다. JDK에 있는 라이브러리 코드를 많이 봐와서 그래서 더 놀랐지 않나 싶다.

아마 라이브러리 또는 프레임워크를 만드는 것과 서비스를 만드는 것은 또 다른 일이겠지. 분야마다, 또는 현업마다 선호하는 컨벤션이 있는 것 같다. 

### 예외 케이스
나왔다 예외 케이스. 내가 이번 미션에서 고민했던거는 [그 예외 처리를 "어디에서" 하느냐](#예외처리에-대하여)였다. 해답이 없어 슬프다.

### 주석은 꼭 필요한 경우만 남긴다.
느끼고 있다. 그 "꼭 필요한 경우"를 판단하기 어려운게 문제지만
...
- 문제는 예외 발생시 재입력을 시도 시킬 때 발생한다.
  - 이름을 입력(첫번째 입력)하고 횟수(두번째 입력)를 입력하게 되어있는데, 횟수 입력에서 예외가 발생하면 다시 이름을 입력하게끔 된다.
  - 이런 증상을 막기 위해서 생각한 방법 중 하나는 플래그든, 어떤 조건이든 그에 대한 변수를 추가하고, 조건이 만족하면 입력을 패스하는 것.
  - 코드의 흐름도 복잡해지는데, 이런 분기까지 추가해야할까?
- 예외를 `Car`가 아닌 입출력 클래스에서 한다면?
  - 문제는 쉬워지고 깔끔하진다.
  - 코드의 흐름도, 입력 요구 반복도.
- 하지만 콘솔이 아닌 다른 방법으로 입력을 받는다면?
  - `RaceIOInterface` 구현하는 다른 클래스를 이용한다면?
  - 그 안에서도 "같은 예외처리"를 또 해줘야 하는가?
  - 이게... 옳은가?
- 결국 지금은 입출력 클래스에서 예외처리를 하였다.
  - 찜찜하지만 어쩔수 없지. 현재 상태에서 보다 깔끔하게 구현할 수 있는 방법을 택했다.
