...
우연히도 이번에는 배열 대신 `List`로 구현한 부분이 꽤 있다. 평소에는 내가 충분히 이해하고 메모리를 컨트롤 할 수 있는 상황이라 판단하고 배열을 쓰는 것을 선호한다. 성능상 효율이 좋기 때문이다. 물론 내가 짜고 있는 레벨에서는 그런 최적화는 필요없는 수준이라는걸 알고 있지만... 웬지모르게 닭잡는데 '소 잡는 칼로 닭 잡는' 기분을 떨칠수가 없다. 하지만 현재 공부하고 있는 [이펙트브 자바]에서도 그렇고 지금도 그렇고 여러곳에서 컬렉션을 쓰는 것을 권장한다. 아마 안정성과 범용성, 확장성면에서 너 나은 선택이기 떄문이겠지...(제네릭이라던가) 앞으로는 조금 더 생각해보고 사용해야겠다.

### 함수(메서드) 라인에 대한 기준
피드백에서는 말한다.
> "공백 주석도 한 라인이다. 주석도 되도록 코드 우측에 달아라."

나름 신선한 충격이었다. 이런 컨벤션에 대해서는 딱히 나만의 무언가를 가지고 있자는 않았지만, 조금 의외인 감이 있었다. JDK에 있는 라이브러리 코드를 많이 봐와서 그래서 더 놀랐지 않나 싶다.
